...
names(logmt)
describe(logmt2$soc)
wtd.table(logmt2$bpl2, weights=logmt$qex)
describe(logmt2$bpl2)
sum(logmt$bpl2[!is.na(logmt$bpl2)] < 50000)
...
gc()
#residence.out.nnd <- NND.hotdeck(data.rec=erf,data.don= pat,match.vars=matchvars, don.class=classes, gdist.fun="Gower")
#residence.out.nnd <- NND.hotdeck(data.rec=erf,data.don= pat,match.vars=matchvars, don.class=classes, gdist.fun="Gower", constrained= TRUE, constr.alg= "relax")
